...
class GetFinancialDetailsConnector @Inject()(httpClient: HttpClientV2,
                                             appConfig: AppConfig)
  ...
  def getFinancialDetails(enrolmentKey: EnrolmentKey, overridingParameters: Option[String])(implicit hc: HeaderCarrier): Future[GetFinancialDetailsResponse] = {
    ...
    httpClient.get(new URL(getFinancialDetailsUrl(enrolmentKey) + parameters)).setHeader(headers *).execute[GetFinancialDetailsResponse].recover {
  ...
  def getFinancialDetailsForAPI(enrolmentKey: EnrolmentKey,
    ...
    httpClient.get(new URL(getFinancialDetailsUrl(enrolmentKey) + queryParams)).setHeader(headers *).execute.recover {
      case e: UpstreamErrorResponse =>

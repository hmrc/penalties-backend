...
import connectors.parsers.ComplianceParser.{CompliancePayloadFailureResponse, CompliancePayloadResponse, CompliancePayloadSuccessResponse}
...
class ComplianceConnector @Inject()(httpClient: HttpClientV2,
                                   appConfig: AppConfig)
  ...
  def getComplianceData(enrolmentKey: EnrolmentKey, fromDate: String, toDate: String)(implicit hc: HeaderCarrier): Future[CompliancePayloadResponse] = {
    ...
    httpClient.get(new URL(url)).setHeader(desHeaders *).execute[CompliancePayloadResponse].recover {

...
The backend for the income tax penalties services such as `income-tax-penalties-frontend`. It handles the following:
...
The service manager configuration name for this service is `PENALTIES_BACKEND`.
But it can be started with`sm --start INCOME_TAX_PENALTIES_ALL` along with dependent services such as `income-tax-penalties-stubs` and more.
It can also be started specifically with `sm --start PENALTIES_BACKEND`.

To run local changes you will have to stop the service in service manager using `sm --stop PENALTIES_BACKEND` then run the script mentioned below.

This application runs on port 9186 .
